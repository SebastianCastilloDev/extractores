...
const ignoredDirectories = ['node_modules', '.next', '.git', 'imprenta'];
const ignoredFiles = [
    '.DS_Store', 
    'package-lock.json', 
    // '.eslintrc.json',
...
const outputFilePath = path.join(__dirname, 'frontend.txt');
...
function traverseDirectory(directory: string, depth = 0, maxDepth = Infinity) {
  ...
  fs.readdirSync(directory, { withFileTypes: true }).forEach((dirent: any) => {
...
async function selectDirectory(currentPath: string, depth: number): Promise<{ selectedDirectory: string }> {
  const directories = fs.readdirSync(currentPath, { withFileTypes: true })
    .filter((dirent: any) => dirent.isDirectory() && !ignoredDirectories.includes(dirent.name))
